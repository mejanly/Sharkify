...
string SharkMesh::nextToken(char delimit, FILE* readFile) {
	...
	while(!feof(readFile)) {
		if(ferror(readFile)) { 
		   printf("1Error reading FILE\n");
		   exit(-1); 
		}
		char cur = fgetc(readFile);
		if(ferror(readFile)) { 
		   printf("2Error reading FILE\n"); 
		   exit(-1);
		}
		if(cur == delimit || cur == '\n') {
		   break;
...
void SharkMesh::restPosition() {
	...
	for(im = vertices.begin(); im != vertices.end(); im++) {
		im->second->transformed = Vector3f(0,0,0);
	}

	skinTransforms = map<string, Matrix>();  //wipe out bone information
}
...
void SharkMesh::linearBlendTransform(Matrix matrix, string boneName) {
	map<Vector3f, SharkVertex*, compareVect3>::iterator im;
	for(im = vertices.begin(); im != vertices.end(); im++) {
...
void SharkMesh::linearBlendTransform() {
	...
	for(im = vertices.begin(); im != vertices.end(); im++) {
		string boneName;
		map<string, Matrix>::iterator ib;
		for(ib = skinTransforms.begin(); ib != skinTransforms.end(); ib++) {
			float weight = (*im).second->checkBone((*ib).first);
				Matrix matrix = gSkinMatrix(ib->first); 
				Matrix scale = Matrix(); //weight mess ups
				(*im).second->transformed += 
					matrix.multScalar(weight).multVec((*im).second->local, true);
		}
	}
