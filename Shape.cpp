...
Shape::Shape(vector<tinyobj::material_t> _materials) {
...
float Shape::randFloat(float l, float h) {
   float r = rand() / (float)RAND_MAX;
   return (1.0f - r) * l + r * h;
}

void Shape::genExplodeVars() {
	explodeVec = glm::vec3(randFloat(-10.0f, 10.0f), randFloat(-10.0f, 10.0f), randFloat(-0.0f, 0.0f));
	velocity = randFloat(-0.3f, 0.3f);
	exploding = true;
	xrot = rand() % 360 + 1;
	yrot = rand() % 360 + 1;
	zrot = rand() % 360 + 1;
