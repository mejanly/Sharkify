...
/*
vector<SharkBone *> SharkMesh::chooseBones(SharkBone *bone) {
   vector<SharkBone *>::iterator ic;
   vector<SharkBone *> childBones = bone->gBones();

   vector<SharkBone *> boneSet;

   for(ic = childBones.begin(); ic != childBones.end(); ic++) {
      if(std::find(bones.begin(), bones.end(), (*ic)) == bones.end()) {
         // bones does not contain child bone, add it to the new vector
         printf("%s\n", ((*ic)->gName()).c_str());

         boneSet.push_back((*ic));
      }
   }
